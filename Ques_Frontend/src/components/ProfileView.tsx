...
export function ProfileView({ userProfile, onUpdate }: ProfileViewProps) {
  ...
  const chinaRegions: { [key: string]: string[] } = {
    '北京': ['北京'],
    '上海': ['上海'],
    '天津': ['天津'],
    '重庆': ['重庆'],
  ...
  const provinces = Object.keys(chinaRegions);
  ...
  const startEditing = (sectionId: string) => {
    ...
    if (sectionId === 'basic-info') {
      ...
      // Initialize province and city from existing location
      if (userProfile.location) {
        const locationParts = userProfile.location.split(', ');
        if (locationParts.length === 2) {
          // Format: "城市, 省份"
          setSelectedCity(locationParts[0]);
          setSelectedProvince(locationParts[1]);
        } else {
          // Direct municipality: just city name
          const city = locationParts[0];
          for (const [province, cities] of Object.entries(chinaRegions)) {
  ...
  const cancelEditing = (sectionId: string) => {
    ...
    // Reset location selection states when canceling basic-info editing
    if (sectionId === 'basic-info') {
      setSelectedProvince('');
      setSelectedCity('');
    }
  ...
  const getSectionKeys = (sectionId: string): (keyof UserProfile)[] => {
    switch (sectionId) {
      case 'basic-info':
        return ['name', 'birthday', 'gender', 'location', 'oneSentenceIntro', 'hobbies', 'languages'];
      case 'skills':
        return ['skills'];
      case 'resources':
        return ['resources'];
  ...
  const handleProvinceChange = (province: string) => {
    setSelectedProvince(province);
    setSelectedCity('');
    setTempValues(prev => ({ ...prev, location: '' }));
  };
  ...
  const handleCityChange = (city: string) => {
    ...
    setTempValues(prev => ({ ...prev, location: locationString }));
  };
      ...
      <div className="grid grid-cols-3 gap-3 items-start">
        ...
        <div className="col-span-2 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <Label className="text-sm">{t('profile.birthday')} *</Label>
              <Input
                value={tempValues.birthday || ''}
                onChange={(e) => updateTempValue('birthday', e.target.value)}
                type="date"
