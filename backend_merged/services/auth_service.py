...
class AuthService:
    ...
    async def create_user_with_phone(
        ...
        existing_auth = db.query(UserAuth).filter(
            ...
            UserAuth.provider_id == phone_number,
            UserAuth.is_verified == True
        ).first()
        ...
        user = User(
            ...
            is_active=True,
            verification_status="verified"  # Phone is already verified via SMS
        )
        
        # Set optional profile fields if provided
        if user_data.get('email'):
            # Note: You might want to add email field to User model if not exists
            pass
        
        if user_data.get('gender'):
            # Note: You might want to add gender field to User model if not exists  
            pass
            
        if user_data.get('date_of_birth'):
            # Note: You might want to add date_of_birth field to User model if not exists
            pass
        ...
        # Create phone authentication record
        password_hash = self.hash_password(password)
        user_auth = UserAuth(
            user_id=user.user_id,
            ...
            password_hash=password_hash,
            is_verified=True,  # Phone was verified via SMS
            is_primary=True,
            created_at=datetime.utcnow(),
        ...
        logger.info(f"User registered successfully with phone: {phone_number}")
        
        return user, access_token, refresh_token
    
    def authenticate_user_phone(self, db: Session, phone_number: str, password: str) -> Optional[User]:
        ...
        # Check password hash exists and verify it
        if not user_auth.password_hash or not self.verify_password(password, user_auth.password_hash):
            logger.warning(f"Password verification failed for phone: {phone_number}")
            return None
        ...
        logger.info(f"User authenticated successfully with phone: {phone_number}")
        return user_auth.user
    ...
    def send_password_reset_email(self, db: Session, email: str) -> bool:
        ...
        try:
            ...
            user_auth = db.query(UserAuth).filter(
                UserAuth.provider_data.contains({"email": email}),
                UserAuth.provider_type == ProviderType.EMAIL
            ).first()
            ...
            db.query(VerificationCode).filter(
                VerificationCode.user_id == user_auth.user_id,
    ...
    def reset_password_with_code(self, db: Session, email: str, reset_code: str, new_password: str) -> bool:
        ...
        try:
            ...
            verification_code = db.query(VerificationCode).filter(
                VerificationCode.user_id == user_auth.user_id,
                VerificationCode.code == reset_code,
                VerificationCode.purpose == "password_reset",
                VerificationCode.expires_at > datetime.utcnow()
            ).first()
            ...
            provider_data = user_auth.provider_data.copy()
            provider_data["password"] = hashed_password
            user_auth.provider_data = provider_data
            ...
            db.query(RefreshToken).filter(
                RefreshToken.user_id == user_auth.user_id
            ).delete()
