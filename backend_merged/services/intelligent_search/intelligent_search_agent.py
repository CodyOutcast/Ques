...
# Import GLM-4 client and Tencent VectorDB adapter
from .glm4_client import GLM4Client, GLM4Model, ResponseFormat
from .tencent_vectordb_adapter import TencentVectorDBAdapter
...
class SearchAgent:
    ...
    def __init__(
        ...
        vectordb_adapter: TencentVectorDBAdapter = None,
        collection_name: str = "user_vectors",
        ...
        Args:
            ...
            vectordb_adapter: Tencent VectorDB adapter instance
            collection_name: Vector collection name
            glm_model: GLM-4 model name
            api_base_url: Database API base URL
            qdrant_url: Qdrant server URL (optional, for hybrid search with fusion)
            qdrant_api_key: Qdrant API key (optional)
            use_qdrant_fusion: Whether to use Qdrant for hybrid search fusion
        """
        ...
        # Initialize Tencent VectorDB adapter (for storage and enrichment)
        self.vectordb = vectordb_adapter or TencentVectorDBAdapter(
            collection_name=collection_name
        )
        self.collection_name = collection_name
        
        # Initialize Qdrant client (for hybrid search with fusion)
        self.use_qdrant_fusion = use_qdrant_fusion
        self.qdrant_client = None
        if use_qdrant_fusion and qdrant_url:
            try:
                ...
                print(f"✅ Qdrant client connected: {qdrant_url}")
            except Exception as e:
                print(f"⚠️  Qdrant connection failed: {e}")
                print("   Falling back to Tencent VectorDB only (dense vectors)")
                self.use_qdrant_fusion = False
        else:
            print("ℹ️  Qdrant fusion disabled, using Tencent VectorDB only")
    ...
    def _initialize_embedding_models(self):
        ...
        # Initialize BGE-M3 dense vector model (required)
        try:
        ...
        # Initialize SPLADE sparse vector model (optional)
        try:
            ...
            print(f"  ✅ SPLADE loaded successfully (device: {self._device})")
            
        except Exception as e:
            print(f"  ⚠️  SPLADE not available (will use dense-only search): {str(e)[:100]}")
            # Set to None - system will fall back to dense-only search
...
"""
    ...
    async def hybrid_search(
        ...
        """
        try:
            # Build Qdrant filter for excluding viewed/swiped users (matching original algorithm)
            from qdrant_client import models
            qdrant_filter = None
            if viewed_user_ids and self.use_qdrant_fusion and self.qdrant_client:
            ...
            # Build filter conditions for Tencent VectorDB (pass viewed_user_ids info)
            filter_conditions = {"viewed_user_ids": viewed_user_ids} if viewed_user_ids else None
            
            # Execute vector search
            if search_strategy == "standard":
                vector_results = self._standard_search(dense_query, sparse_query, qdrant_filter, limit)
            elif search_strategy == "expanded":
                vector_results = self._expanded_search(dense_query, sparse_query, qdrant_filter, limit)
            elif search_strategy == "custom":
                vector_results = self._custom_search(dense_query, sparse_query, qdrant_filter, limit)
            else:
                raise ValueError(f"Unsupported search strategy: {search_strategy}")
            
            # Filter out viewed users from results if not using Qdrant (post-processing filter)
            if viewed_user_ids and vector_results:
                vector_results = [
                    result for result in vector_results 
                    if str(result.get("user_id")) not in viewed_user_ids
                ]
    ...
    def _standard_search(
        ...
        """Standard search strategy - prefetch limit=50, use DBSF fusion, then filter swiped users"""
        try:
            ...
            # If Qdrant fusion is enabled, use it for hybrid search
            if self.use_qdrant_fusion and self.qdrant_client:
                # Step 1: Fetch 50 closest users using DBSF fusion
                return self._qdrant_hybrid_search(
                    ...
                    limit=limit,
                    fusion_type="DBSF",
                    prefetch_limit=50  # Fetch 50 users, then filter viewed/swiped, pick top 10
                )
            
            # Otherwise, fall back to dense-only search in Tencent VectorDB
            results = self.vectordb.search(
                dense_vector=dense_vec,
                sparse_vector=None,
                limit=limit * 5,  # Fetch more to account for filtering
                filter_conditions=None,  # Handle filtering in post-processing
                use_hybrid=False,
                alpha=0.7
            )
            
            return results[:limit]  # Return top results after filtering
        except Exception as e:
    ...
    def _expanded_search(
        ...
        """Expanded search strategy - prefetch limit=150, use RRF fusion, then filter swiped users"""
        try:
            # Use preloaded dense vector model
            if self._dense_model is None:
                from sentence_transformers import SentenceTransformer
            ...
            # If Qdrant fusion is enabled, use it for hybrid search
            if self.use_qdrant_fusion and self.qdrant_client:
                # Step 1: Fetch 150 closest users using RRF fusion
                return self._qdrant_hybrid_search(
                    dense_vec=dense_vec,
                    sparse_query=sparse_query,
                    qdrant_filter=qdrant_filter,
                    limit=limit,
                    fusion_type="RRF",
                    prefetch_limit=150  # Fetch 150 users, then filter viewed/swiped, pick top 10
                )
            
            # Use Tencent VectorDB adapter with larger search space
            results = self.vectordb.search(
                dense_vector=dense_vec,
                sparse_vector=None,
                limit=limit * 15,  # Fetch 15x more to account for broader search + filtering
                filter_conditions=None,  # Handle filtering in post-processing
                use_hybrid=False,
                alpha=0.7
            )
    ...
    def _custom_search(
        ...
        """Custom search strategy - same as standard for now"""
        return self._standard_search(dense_query, sparse_query, qdrant_filter, limit)
    
    def _build_splade_sparse_vector(self, text: str) -> List[float]:
        """Build SPLADE sparse vector (simplified version)"""
        try:
            ...
            import torch
            # Tokenize
            inputs = self._splade_tokenizer(text, return_tensors="pt", truncation=True, max_length=512)
            inputs = {k: v.to(self._device) for k, v in inputs.items()}
            
            # Generate sparse representation
            with torch.no_grad():
                output = self._splade_model(**inputs)
            ...
            # Convert to list
            return sparse_vec.cpu().tolist()
        except Exception as e:
            print(f"Failed to build sparse vector: {e}")
            return []
    ...
    def _qdrant_hybrid_search(
        ...
        qdrant_filter: Optional[Any],
        limit: int,
        fusion_type: str = "DBSF",
        prefetch_limit: int = 50
    ) -> List[Dict]:
        """
        Hybrid search using Qdrant for fusion, then enrich with Tencent VectorDB data
        
        Workflow:
        1. Compute dense vector (BGE-M3) - already passed in
        2. Compute sparse vector (SPLADE)
        3. Use Qdrant for hybrid search with fusion (DBSF or RRF)
        4. Get user IDs from Qdrant results
        5. Fetch full data from Tencent VectorDB using user IDs
        
        Args:
            dense_vec: Pre-computed dense vector
            sparse_query: Query text for sparse vector
            filter_conditions: Filter conditions (converted to Qdrant format)
            limit: Number of results
            fusion_type: "DBSF" or "RRF"
        
        Returns:
            List of search results with full payload from VectorDB
        """
        try:
            from qdrant_client import models
            
            # Step 1: Generate sparse vector
            sparse_vec = self._build_splade_sparse_vector(sparse_query)
            if not sparse_vec:
                print("⚠️  Sparse vector generation failed, falling back to dense-only")
            ...
            # Convert sparse vector list to Qdrant SparseVector format
            # Only keep non-zero indices
            indices = [i for i, val in enumerate(sparse_vec) if val > 0]
            values = [sparse_vec[i] for i in indices]
            qdrant_sparse_vec = models.SparseVector(indices=indices, values=values)
            
            # Step 2: Use provided Qdrant filter (already in correct format)
            # qdrant_filter is passed directly from hybrid_search method
            
            # Step 3: Execute hybrid search with fusion in Qdrant
            print(f"🔍 Executing Qdrant hybrid search (fusion={fusion_type})...")
            
            fusion_method = models.Fusion.DBSF if fusion_type == "DBSF" else models.Fusion.RRF
            
            result = self.qdrant_client.query_points(
                collection_name=self.collection_name,
                prefetch=[
                    {"query": qdrant_sparse_vec, "using": "sparse", "limit": prefetch_limit},
            ...
            # Step 4: Extract user IDs from Qdrant results
            user_ids = [str(hit.id) for hit in result.points]
            print(f"✅ Qdrant returned {len(user_ids)} results")
            
            # Step 5: Fetch full data from Tencent VectorDB
            # Option A: If VectorDB has the full data, query by user_id
            # Option B: Use Qdrant payload directly
            
            # For now, use Qdrant payload and format to match expected structure
            formatted_results = []
            ...
            print(f"✅ Hybrid search completed: {len(formatted_results)} results")
            return formatted_results
            
        except Exception as e:
            print(f"❌ Qdrant hybrid search failed: {e}")
            print("   Falling back to Tencent VectorDB dense-only search")
            return self.vectordb.search(
                dense_vector=dense_vec,
                limit=limit,
                filter_conditions=None,
    ...
    # ===== Legacy code removed, using Tencent VectorDB adapter instead =====
        ...
        try:
            ...
            for attempt, strategy in enumerate(search_strategies, 1):
                ...
                candidates = await self.hybrid_search(
                    ...
                    limit=50,  # Changed from 10 to 50 to get more candidates
